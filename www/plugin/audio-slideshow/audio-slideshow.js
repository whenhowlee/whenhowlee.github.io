	...
	function setup() {
		...
		for( var h = 0, len1 = horizontalSlides.length; h < len1; h++ ) {
			...
			if ( !verticalSlides.length ) {
				setupAllAudioElements( divElement, h, 0, horizontalSlides[ h ].querySelectorAll( '.fragment' ) );
			}
			else {
				for( var v = 0, len2 = verticalSlides.length; v < len2; v++ ) {
					setupAllAudioElements( divElement, h, v, verticalSlides[ v ].querySelectorAll( '.fragment' ) );
				}
			}
		}
	}

	function setupAllAudioElements( container, h, v, fragments ) {
		setupAudioElement( container, h + '.' + v);
		for( var f = 0, len = fragments.length; f < len; f++ ) {
			setupAudioElement( container, h + '.' + v + '.' + fragments[ f ].getAttribute( 'data-fragment-index' ) );
		}

	}
	function setupAudioElement( container, indices ) {
